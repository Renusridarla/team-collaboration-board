...
export default function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(v => !v)} className="relative rounded-full border border-slate-800 p-2 text-slate-300 hover:bg-slate-800">
        <Bell size={18} />
        {unreadCount > 0 && <span className="absolute -right-1 -top-1 inline-flex items-center justify-center rounded-full bg-rose-500 px-1.5 py-0.5 text-xs text-white">{unreadCount}</span>}
      </button>

      {open && (
        <div className="absolute right-0 z-50 mt-2 w-80 rounded-lg border border-slate-700 bg-slate-900/90 p-3">
          <div className="flex items-center justify-between">
            <div className="text-sm font-medium text-white">Notifications</div>
            <button onClick={() => { setOpen(false); }} className="text-xs text-slate-400">Close</button>
          </div>
          <div className="mt-3 space-y-2 max-h-64 overflow-auto">
            {notifications.length === 0 && <div className="text-sm text-slate-400">No notifications</div>}
            {notifications.map(n => (
              <div key={n._id} className={`rounded-md p-2 ${n.isRead ? 'bg-slate-800' : 'bg-slate-700'}`}>
